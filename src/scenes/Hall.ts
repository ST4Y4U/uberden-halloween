...
// UI가 항상 파이 위에 뜨도록 고정
const DEPTH = { BG:-1000, CLIENT:10, COUNTER:12, BOARD:13, PIE:40, UI:80, CHOICE:90 };
...
export default class Hall extends Phaser.Scene {
  ...
  async create(){
    ...
    if (P?.cooked && !P.delivered) {
      ...
      this.beginOrderDialogue();      // 프리대사 스킵, 주문으로 바로
    } else {
      ...
      if (pre.length) {
        ...
        this.beginOrderDialogue();    // 프리대사 없으면 주문으로
      }
    }
  }
  ...
  private playDialogNode(id:string){
    ...
    this.client.setTexture(this.getClientSprite(node.sprite ?? "standard"));
    this.myBox.setVisible(false); this.myText.setVisible(false);
    ...
    if (node.choices?.length){
      ...
      for (const ch of node.choices){
        const btn = this.add.text(640, y, ch.label, {
          fontFamily:"sans-serif", fontSize:"24px", color:"#F7E2B2", backgroundColor:"#6E2B8B"
  ...
  private afterDeliver(ok: boolean){
    ...
    const proceed = () => {
      if (this.stageData.endGame) {
        ...
        if (ep.length) {
          this.playEpilogue(ep, ()=> this.scene.start("Result", { result: computeEnding() }));
        } else {
          this.scene.start("Result", { result: computeEnding() });
        }
      } else {
        if (ok) advanceStage();
    ...
    this.showClientLine(line.text, line.sprite ?? "standard", 1400, proceed);
  }
}