...
export default class Hall extends Phaser.Scene {
  ...
  async create() {
    ...
    this.hallBoard = this.add.image(POS.board.x, POS.board.y, "pie_cuttingboard").setOrigin(0.5).setDepth(DEPTH.BOARD).setVisible(true);

    // JSON에 deliverZone이 있으면 그걸 사용
    ...
    if (dz && typeof dz.x === "number") {
      ...
      this.deliverRect = new Phaser.Geom.Rectangle(cx - cb.width*0.45, cy - cb.height*0.45, cb.width*0.9, cb.height*0.9);
    }
    ...
    this.toKitchenArrow = this.add.image(POS.arrowKitchen.x, POS.arrowKitchen.y, "hall_arrow").setDepth(DEPTH.UI+5).setInteractive({ useHandCursor:true }).on("pointerup", () => this.scene.start("Stage"));
  ...
  private getOutcomeLine(type: "success" | "fail"): { text: string; sprite: "standard"|"happy"|"angry" } {
    ...
    return type === "success" ? { text: "고마워. 딱 내가 원하던 파이야.", sprite: "happy" } : { text: "…이건 주문이 아니야.", sprite: "angry" };
  }

  private showClientLine(line: { text: string; sprite: "standard"|"happy"|"angry" }, ms=700, onDone?: ()=>void){
    this.client.setTexture(this.getClientSprite(line.sprite));
    this.clBox.setVisible(true);
    this.clText.setVisible(true).setText(line.text);
    if (ms>0) this.time.delayedCall(ms, ()=>{ this.clBox.setVisible(false); this.clText.setVisible(false); onDone?.(); });
  }
  ...
  private spawnHallPie(filling: string | null, lattice: boolean, toppings: string[]){
    ...
    for (const t of toppings) g.add(this.add.image(PIE_OFFSET.x, PIE_OFFSET.y, t).setDepth(DEPTH.PIE+1));

    g.setSize(PIE_HIT.w, PIE_HIT.h);
    g.setInteractive(new Phaser.Geom.Rectangle(-PIE_HIT.w/2+60, -PIE_HIT.h/2, PIE_HIT.w, PIE_HIT.h), Phaser.Geom.Rectangle.Contains); // 오른쪽으로 60px 이동
    this.input.setDraggable(g, true);

    g.on("drag", (_p:any, dx:number, dy:number)=>g.setPosition(dx, dy));

    g.on("dragend", ()=>{
      const r = g.getBounds();
      const hit = Phaser.Geom.Intersects.RectangleToRectangle(r, this.deliverRect);
      if (!hit) {
        this.tweens.add({ targets: g, x: POS.hallPie.x, y: POS.hallPie.y, duration: 200 });
        return;
      }
      const ok = this.evaluatePie();
      if (ok) {
        const line = this.getOutcomeLine("success");
        this.showClientLine(line, 1000, ()=>this.afterDeliver(true));
      } else {
        const line = this.getOutcomeLine("fail");
        this.showClientLine(line, 1000, ()=>this.afterDeliver(false));
      }
    });
  ...
  private evaluatePie(): boolean {
    ...
    const fillingOk = o.filling ? normalize(pie.filling) === normalize(o.filling) : true;
    const latticeOk = o.ignoreLattice || (o.needsLattice === undefined ? true : pie.lattice === !!o.needsLattice);
    const toppingOk = o.ignoreToppings || (o.toppings ? o.toppings.every((t:string)=>pie.toppings.includes(t)) : true);
